...
// automatically gets the verified badge next to their name everywhere — no
...
function verifiedBadgeHtml(username){
  if(!isOfficialAccount(username)) return '';
  return `<svg class="verified-tick" viewBox="0 0 22 22" width="15" height="15" aria-label="Verified"><path fill="currentColor" d="M11 0l2.39 1.7 2.87-.5 1.19 2.64 2.64 1.19-.5 2.87L21.29 10l-1.7 2.39.5 2.87-2.64 1.19-1.19 2.64-2.87-.5L11 20l-2.39-1.7-2.87.5-1.19-2.64-2.64-1.19.5-2.87L.71 10l1.7-2.39-.5-2.87 2.64-1.19L5.74 1.2l2.87.5L11 0z"/><path fill="var(--bg)" d="M9.3 14.2L5.6 10.5l1.4-1.4 2.3 2.3 5-5 1.4 1.4z"/></svg>`;
}
  ...
  try{
    ...
    list.innerHTML = rows.map((u,i) => {
      ...
      return `<div class="lb-row clickable" data-uid="${u.uid}">
        ...
        <div class="lb-name-wrap"><div class="lb-name" style="display:inline-flex; align-items:center; gap:4px;">${escapeHtml(displayName)}${verifiedBadgeHtml(u.username)}</div>${subLine}</div>
...
function renderPrayerList(){
  ...
  list.querySelectorAll('.toggle').forEach(t => {
    t.addEventListener('click', () => {
      ...
      state.notifyOn[key] = !state.notifyOn[key];
  ...
  document.getElementById('btnFinishSignup').addEventListener('click', async () => {
    ...
    try{
      ...
      await db.collection('users').doc(uid).set({
        ...
        hideFollowing: false, hideNamazStreak: false, hideDailyStreak: false, hideTotalNamaz: false, warnings: 0, banned: false,
    ...
    <div class="post-card ${isVerifiedShare ? 'verified-streak' : ''}" data-post-id="${p.id}">
      <div class="post-head">
        <div class="post-author clickable" data-uid="${p.uid}">
          ...
          <div><div class="post-uname">${escapeHtml(displayName)}${verifiedBadgeHtml(p.username)}</div><div class="post-time">${timeAgo(p.createdAt)}</div></div>
        </div>
      ...
      ${p.imageURL ? `<img class="post-image" src="${p.imageURL}" style="object-position:${p.imageFocusX ?? 50}% ${p.imageFocusY ?? 50}%" alt="${escapeHtml(p.title||'Post image')}" data-action="open-image" data-id="${p.id}">` : ''}
...
function renderComments(comments){
  ...
  const rowHtml = (c, isReply) => {
    ...
    return `<div class="comment-row ${isReply?'comment-reply':''}">
      ...
      <div style="flex:1;">
        <div class="comment-uname clickable" data-uid="${c.uid}">${escapeHtml(displayName)}${verifiedBadgeHtml(c.username)}</div>
...
async function renderProfileCard(el, p, isOwn, token){
  ...
  el.innerHTML = `
    <div class="profile-header">
      ...
      <div class="faida-title" style="display:inline-flex; align-items:center; justify-content:center; gap:5px;">${escapeHtml(displayName)}${verifiedBadgeHtml(p.username)}</div>
...
async function performFullAppReset(){
  ...
  try{
    ...
    await db.collection('users').doc(uid).set({
      namazStreak: 0, namazBest: 0, dailyStreak: 0, dailyBest: 0, warnings: 0, banned: false,
    }, { merge:true }).catch(() => {});
  ...
  try{
    ...
    if(topUsers.length){
      html += `<div class="section-label" style="margin-top:10px;">Users</div>` + topUsers.map(u => {
        ...
        const initial = name.charAt(0).toUpperCase();
        const avatar = u.photoURL ? `<img src="${u.photoURL}">` : initial;
        return `<div class="search-user-row" data-uid="${u.uid}"><div class="search-user-avatar">${avatar}</div><div><div style="display:inline-flex; align-items:center; gap:4px;">${escapeHtml(name)}${verifiedBadgeHtml(u.username)}</div><div style="font-size:11px; color:var(--text-muted);">@${escapeHtml(u.username && !u.username.includes('@') ? u.username : 'user')}</div></div></div>`;
      }).join('');
    }
...
function bindInviteFriends(){
  document.getElementById('btnInviteFriends').addEventListener('click', async () => {
    const url = window.location.href.split('?')[0].split('#')[0];
    const text = 'Pak Raho app try karein — Namaz timings, duaein, streak tracker aur Islamic community, sab ek jagah!';
    if(navigator.share){
      try{ await navigator.share({ title:'Pak Raho', text, url }); } catch(e){}
    } else {
      try{
